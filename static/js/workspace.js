...
export function syncWorkspaceIndicator(path) {
  ...
  if (pill) {
    ...
    if (path) pill.title = `Workspace: ${path}\nFile tools are confined here; shell commands start here but are not sandboxed unless you enable it in Settings > Agent (Docker required) — otherwise they can reach outside it.\nClick to clear.`;
  }
...
function _getModal() {
  ...
  _modal.innerHTML = `
    <div class="modal-content">
      ...
      <p class="muted workspace-note">File tools are <strong>confined</strong> to this folder. Shell commands start here but are <strong>not sandboxed</strong> by default and can reach outside it — enable Docker sandboxing in Settings &gt; Agent to confine them too. Without that, a workspace scopes the tools; it is not a security boundary.</p>
